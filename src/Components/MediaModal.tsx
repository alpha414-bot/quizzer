...
const MediaModal: React.FC<{
  ...
  return (
    <div
      ...
      tabIndex={-1}
      aria-hidden="true"
      className="hidden overflow-y-auto bg-yellow-400/10 overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full"
    >
      <div className="relative p-4 w-full max-w-2xl h-full md:h-auto">
        {/* Modal content */}
        <div className="relative text-center bg-white rounded-lg shadow sm:p-5">
          <div className="">
            <div>
              {/* Tab List */}
              ...
              <div id="media-tab-content">
                {mediaType.map((media_name, index) => {
                  return (
                    <div
                      key={index}
                      className="hidden p-2 rounded-lg bg-gray-50"
                      id={`${media_name}_tab`}
                      role="tabpanel"
                      aria-labelledby={`${media_name}-tab`}
                    >
                      {(_.includes(mediaType, "image") && (
                        <>
                          <div className="py-6 max-h-[30rem] overflow-y-auto">
                            <div className="grid grid-col grid-cols-3 flex-wrap gap-3">
                              {(isMediaUploading && (
                                <>
                              ...
                              {medias &&
                                medias
                                  ...
                                  .map((item, index) => (
                                    <MediaComponent key={index} item={item} />
                                  ))}
                            </div>
                          </div>
                        </>
